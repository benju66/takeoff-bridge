...
// Imported pool: select().eq().neq("item_id").neq("data_fidelity") awaited —
// the second neq is the fidelity Phase 2 lump exclusion.
const mockNeq = vi.fn();
const mockNeqItemId = vi.fn(() => ({ neq: mockNeq }));
...
describe("Estimate Versions — getBidPriceHistory", () => {
  const submittedVersionRow = {
    ...
    line_items: [
      // Real observation: carried dollars in the official bid. The frozen
      // payload (buildLineItemPayload) always carries matched_qty +
      // data_fidelity — they ride onto the observation for historyTrust.
      { item_id: "03-3000.001", unit_price: 5.5, uom: " sf ", total: 550, matched_qty: 100, data_fidelity: "discrete_unit" },
      // Untouched template scaffold: default price, zero total → excluded.
      { item_id: "04-0000.001", unit_price: 9.99, uom: "SF", total: 0, matched_qty: 0, data_fidelity: "discrete_unit" },
      // No item code → excluded.
      { item_id: "", unit_price: 3, uom: "EA", total: 300, matched_qty: 100, data_fidelity: "discrete_unit" },
    ],
