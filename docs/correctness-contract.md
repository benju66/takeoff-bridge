...
cost-per-SF/unit are computed on) is guarded today by
`INV-1 single-total: reported subtotal is the modifier basis`. The full cross-layer tie-out
(engine → export rollup, on a real bid) is the **Phase 2 golden harness**
(`src/__tests__/golden-mckenna.test.ts`) and is recorded here as `it.todo`.
...
|---|---|---|
| INV-1 Single total | engine half today; full Phase 2 | `INV-1 single-total: reported subtotal is the modifier basis`; full tie-out = `golden-mckenna.test.ts` (`it.todo`) |
