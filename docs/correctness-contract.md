...
| # | Finding | Disposition |
|---|---------|-------------|
| **G-1** | **Excel applies NO rounding** to its modifier/total cells. With a `"dollar"` rounding rule the total would diverge by ~$0.18 on this bid (up to ~$0.50 per modifier). | **(c) Legitimate Excel-vs-JS difference — RESOLVED by B-3 (math-trust slice 6, 2026-06-09): the rounding default is now `"none"`**, so the app ties the unrounded company spreadsheet to the cent out of the box (the harness already ran `roundingRule: "none"`; the default now matches it). Ties to the INV-4 watch-item: a project that *opts into* `"dollar"` rounding via the per-project toggle will *intentionally* differ from an unrounded sheet — that is a setting, not a bug. |
...
engine is not yet tied to a real estimate (B-2 — a born-in-app golden, per G-2). The rounding
default (B-3, per G-1) was **DECIDED and shipped → `"none"`** (math-trust Phase 5 slice 6,
2026-06-09); the per-project toggle remains for projects that want `"dollar"`/`"ten"`/`"hundred"`.
