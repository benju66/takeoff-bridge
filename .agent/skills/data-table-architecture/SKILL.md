...
| `src/components/workspace/OneOffCodeCell.tsx` | **One-off Code-cell assign-and-place (B5 / D1).** The Code column renders this for a one-off row: UNCODED → "⚠ Assign code" → inline validated Procore-code input (`validateOneOffCode`) → the grid's undoable `assignOneOffCode`; CODED → the code (click to re-assign). An uncoded one-off is BLOCKED from export (`validateExportReadiness` pushes a `kind: 'oneOff'` blocker; `useExportHandlers` surfaces a clear, line-named message — the takeoff-row override modal can't fix it). Dismiss = container-ref check (§8 #7). |
