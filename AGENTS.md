...
## Procore Cost Code Authority
* **Single Source of Truth**: The `procore_cost_codes` table (managed via `/procore-codes`, read through `getProcoreCostCodes()`) is the canonical list of valid Procore cost codes and their types (Labor/Material/Subcontract/Equipment). Export and mapping validation MUST resolve validity from this table (via the primed `procoreValidCodes.ts` overlay).
* **JSON is a drift baseline, NOT a source**: `src/lib/procore-valid-codes.json` (generated by `npm run sync-codes` → `scripts/harvest-cost-codes.js`) is retained ONLY as the cold-start fallback and the drift-check baseline (`procore-valid-codes-sync.test.ts`). Agents MUST NOT hand-edit it to change which codes are valid, and MUST NOT re-point validation back to it. To add/retire a Procore code, update the table.
* **No Fabricated Types**: Never invent a Procore `type` for a code the source list does not record (AGENTS.md "No Speculative Changes"). Codes absent from the table are invalid by absence — do not insert placeholder/retired rows with a guessed type.
* **`npm run sync-codes` clobbers `estimate-catalog.json`'s manual additions**: the committed `src/lib/estimate-catalog.json` is `227 = 221 harvested + 6 architect-confirmed manual built-in additions (2026-06-10)` (`03-3543.002`, `07-1000.003`, `09-9000.002`, `26-0000.006`, `32-1613.007`, `01-0230.002`) that are NOT in the template, so the harvest does not regenerate them. Re-running `sync-codes` overwrites the file at 221 and silently drops those 6 (breaking `catalogPriceLookup.test` / `catalogRateSeed.test`, both pinned at 227). After any `sync-codes` run, restore `estimate-catalog.json` and re-apply only genuinely new harvested codes — until the 6 are migrated off the harvested file.
